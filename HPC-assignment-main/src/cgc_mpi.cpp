...
// Compute cluster averages (row × column blocks)
std::vector<double> calculate_cluster_average(
    int num_rows, int num_cols,
    int num_row_labels, int num_col_labels,
    const float* matrix,
    ...
    int rank, int size)
{
    ...
    int count_row = rows_per_proc + (rank < extra_rows ? 1 : 0);
    int end_row = start_row + count_row;
...
// Update row labels (parallel by rows)
std::pair<int,double> update_row_labels(
    int num_rows, int num_cols,
    int num_row_labels, int num_col_labels,
    const float* matrix,
    ...
    int rank, int size)
{
    ...
    int start = rank * cols_per_proc + std::min(rank, extra_cols);
    int count = cols_per_proc + (rank < extra_cols ? 1 : 0);
    int end = start + count;
...
// One clustering iteration (row + column parallelized)
std::pair<int,double> cluster_mpi_iteration(
    int num_rows, int num_cols,
    int num_row_labels, int num_col_labels,
    const float* matrix,
