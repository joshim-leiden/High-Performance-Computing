    ...
    int rank, int size)
{
    ...
    MPI_Allreduce(local_sum.data(), global_sum.data(), global_sum.size(), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(local_count.data(), global_count.data(), global_count.size(), MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
// ---------------------------------------------------
// One iteration (row + column interleaving)
// ---------------------------------------------------
    ...
    int rank, int size)
{
    ...
    int best_iter = 0;
    double best_dist = std::numeric_limits<double>::infinity();
    std::vector<label_type> best_row_labels = row_labels;
    std::vector<label_type> best_col_labels = col_labels;
    ...
    while(iter < max_iterations) {
        ...
        if(rank == 0) {
            std::cout << "Iteration " << iter << " : " << updated << " labels updated\n";
        }

        if(dist < best_dist) {
            best_dist = dist;
            best_row_labels = std::vector<label_type>(row_labels, row_labels + num_rows);
            best_col_labels = std::vector<label_type>(col_labels, col_labels + num_cols);
            best_iter = iter;
        }
    ...
    // Restore best labels
    std::copy(best_row_labels.begin(), best_row_labels.end(), row_labels);
    std::copy(best_col_labels.begin(), best_col_labels.end(), col_labels);

    auto after = std::chrono::high_resolution_clock::now();
    double t = std::chrono::duration<double>(after-before).count();

    if(rank == 0) {
        std::cout << "Best iteration: " << best_iter << "\n";
        std::cout << "Total clustering time: " << t << " sec\n";
    }
}
...
int main(int argc, const char* argv[])
{
    ...
    int num_rows=0, num_cols=0, num_row_labels=0, num_col_labels=0, max_iter=400;

    if(!parse_arguments(argc, argv, &num_rows, &num_cols, &num_row_labels, &num_col_labels,
